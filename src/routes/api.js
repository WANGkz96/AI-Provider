...
const runRateLimiter = createRateLimiter({
  maxRequests: config.runRateLimitMax,
  windowMs: config.runRateLimitWindowMs,
  label: 'run requests'
});

const runConcurrencyLimiter = createConcurrencyLimiter({
  maxConcurrent: config.runConcurrencyLimit,
  label: 'run requests'
});
...
router.get('/auth/check', requireAccessKey, (req, res) => {
  res.json({
    ok: true,
    authenticatedAs: req.authSubject || null
  });
});

router.get('/config', requireAccessKey, (req, res) => {
  if (!config.enableConfigRoutes) {
    return res.status(403).json({ error: 'Config routes are disabled on this server' });
  }
...
router.post('/config', requireAccessKey, (req, res) => {
  if (!config.enableConfigRoutes) {
    return res.status(403).json({ error: 'Config routes are disabled on this server' });
  }
...
router.get('/available-models', requireAccessKey, async (req, res) => {
  const models = getConfiguredModels();
...
router.get('/audio-proxy', requireAccessKey, (req, res) => {
    if (!config.enableAudioProxy) {
        return res.status(404).json({ error: 'Audio proxy is disabled on this server' });
    }

    if (!config.audioProxyRoot) {
        return res.status(500).json({ error: 'AUDIO_PROXY_ROOT is not configured' });
    }

    const requestedPath = typeof req.query.path === 'string' ? req.query.path : '';
    if (!requestedPath) {
        return res.status(400).json({ error: 'Missing audio proxy path' });
    }

    const resolvedPath = path.resolve(config.audioProxyRoot, requestedPath);
    if (!isPathInsideRoot(config.audioProxyRoot, resolvedPath)) {
        return res.status(403).json({ error: 'Requested path is outside AUDIO_PROXY_ROOT' });
    }
